...
class ActiveWhiteListSystem(BasicConfig, BasicSystem):
    ...
    async def _handle_add(self, boardcast_info: BoardcastInfo) -> bool:
        ...
        if not command:
            await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("add_usage"))])
            return True
        
        player = command.split()[0]  # 获取第一个参数作为玩家名
        
        if player in self:
            await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("player_already_in_list", player_name=player))])
            return True

        self[player] = True
        await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("player_added", player_name=player))])
        return True
    ...
    async def _handle_remove(self, boardcast_info: BoardcastInfo) -> bool:
        ...
        if not command:
            await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("remove_usage"))])
            return True
        
        player = command.split()[0]  # 获取第一个参数作为玩家名
        
        if player not in self:
            await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("player_not_in_list", player_name=player))])
            return True

        del self[player]
        await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("player_removed", player_name=player))])
        return True
    ...
    async def _handle_list(self, boardcast_info: BoardcastInfo) -> bool:
        ...
        players_str = "\n".join(players)
        await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("list_content", count=len(players), players=players_str))])
        return True
