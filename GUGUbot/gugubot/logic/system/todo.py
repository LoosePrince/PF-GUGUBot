...
class TodoSystem(BasicConfig, BasicSystem):
    ...
    async def _handle_remove(self, boardcast_info: BoardcastInfo) -> bool:
        ...
        if command not in todos:
            await self.reply(boardcast_info, [MessageBuilder.text(
                self.get_tr("remove_not_exist", id=command)
            )])
            return True
        ...
        await self.reply(boardcast_info, [MessageBuilder.text(
            self.get_tr("remove_success", id=command)
        )])
        return True
    ...
    async def _handle_complete(self, boardcast_info: BoardcastInfo) -> bool:
        ...
        if not command:
            await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("complete_instruction"))])
            return True
        ...
        if command not in todos:
            await self.reply(boardcast_info, [MessageBuilder.text(
                self.get_tr("complete_not_exist", id=command)
            )])
            return True
        ...
        if todo.get("completed", False):
            await self.reply(boardcast_info, [MessageBuilder.text(
                self.get_tr("complete_already", id=command)
            )])
            return True
        ...
        await self.reply(boardcast_info, [MessageBuilder.text(
            self.get_tr("complete_success", id=command)
        )])
        return True
    ...
    async def _handle_undo(self, boardcast_info: BoardcastInfo) -> bool:
        ...
        if not command:
            await self.reply(boardcast_info, [MessageBuilder.text(self.get_tr("undo_instruction"))])
            return True
        ...
        if command not in todos:
            await self.reply(boardcast_info, [MessageBuilder.text(
                self.get_tr("undo_not_exist", id=command)
            )])
            return True
        ...
        if not todo.get("completed", False):
            await self.reply(boardcast_info, [MessageBuilder.text(
                self.get_tr("undo_not_completed", id=command)
            )])
            return True
        ...
        await self.reply(boardcast_info, [MessageBuilder.text(
            self.get_tr("undo_success", id=command)
        )])
        return True
    ...
    async def _handle_list(self, boardcast_info: BoardcastInfo) -> bool:
        ...
        for todo_id in sorted(todos.keys(), key=lambda x: int(x) if x.isdigit() else 0):
            ...
            status_mark = self.get_tr("status_completed") if is_completed else self.get_tr("status_pending")
        ...
        todo_list_parts = []
        if pending_list:
            todo_list_parts.append(self.get_tr("list_pending", todo_list="\n".join(pending_list)))
        if completed_list:
            todo_list_parts.append(self.get_tr("list_completed", todo_list="\n".join(completed_list)))
