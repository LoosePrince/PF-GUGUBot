...
class ArrayHandler:
    @staticmethod
    def build(array: List[Dict[str, Dict[str, str]]]) -> List[Dict[str, Dict[str, str]]]:
        return array
    
    @staticmethod
    def parse(message: List[Dict[str, Dict[str, str]]]) -> List[Dict[str, Dict[str, str]]]:
        return message
    ...
    def parse(message: str) -> List[Dict[str, Dict[str, str]]]:
        ...
        for match in cq_pattern.finditer(message):
            ...
            if start > last_index:
                ...
                if text_part:
                    result.append({
                        'type': 'text',
                        'data': {'text': html.unescape(text_part)}
                    })
            ...
            kv_pairs = {
                ...
                for key, val in kv_pattern.findall(kv_string)
            }
        ...
        if last_index < len(message):
            ...
            if tail_text:
                result.append({
                    'type': 'text',
                    'data': {'text': html.unescape(tail_text)}
                })
