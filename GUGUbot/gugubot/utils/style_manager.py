...
try:
    ...
    class YAMLWrapper:
        ...
        def dump(data, stream):
            return _yaml.safe_dump(data, stream, allow_unicode=True, default_flow_style=False)
...
class StyleManager:
    ...
    def scan_styles(self) -> None:
        ...
        if self.current_style and self.current_style not in self.styles:
            self.server.logger.warning(f"[GUGUBot] 配置的风格 {self.current_style} 不存在，已重置")
    ...
    def set_current_style(self, style_name: str, bypass_cooldown: bool = False) -> Tuple[bool, str, float]:
        """切换当前风格
    ...
    def _register_style_to_mcdr(self, style_name: str) -> None:
        ...
        try:
            ...
            self.server.logger.info(f"[GUGUBot] 已将风格 {style_name} 注册到 MCDR 翻译系统")
        except Exception as e:
